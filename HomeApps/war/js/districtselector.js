TB.form.DistrictSelector = new function() {
	...
	this.attach = function(d, h, g, e) {
		d = $(d);
		h = $(h);
		g = $(g);
		e = TB.applyIf(e || {}, c);
		var f = {};
		f.selectProv = function(j, i) {
			if (j) {
				d.value = j
			}
			h.options.length = 0;
			g.options.length = 0;
			b(h, d.value, e.filter);
			if (typeof i == "undefined" || i == true) {
				if (h.value) {
					this.selectCity(h.value)
				}
			}
		};
		f.selectCity = function(j, i) {
			if (j) {
				h.value = j
			}
			if (typeof i == "undefined" || i == true) {
				g.options.length = 0;
				b(g, h.value, e.filter)
			}
		};
		f.selectDist = function(l, i) {
			if (l == "1") {
				return
			}
			if (l) {
		...
		f.initSelectedCodes = function(i) {
			if (YAHOO.lang.isArray(i)) {
				f.selectProv(i[0] || "", true);
				f.selectCity(i[1] || "", true);
				f.selectDist(i[2] || "", false)
			} else {
		...
		f.getSelectedCodes = function() {
			return [ d.value, h.value, g.value ]
		};
		f.getDivisionText = function(j) {
			var i = [ d, h, g ].map(function(k) {
...
TB.form.DistrictSelector.initData(TB.form.DistrictSelector._tb_ds_data);